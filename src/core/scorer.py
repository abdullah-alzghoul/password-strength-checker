...
from src.core.breach_checker import BreachResult, check_breach
from src.core.compliance import ComplianceCheck, check_all_standards
from src.core.crack_time import CrackTimeEstimate, estimate_crack_times
from src.core.entropy import EntropyResult, StrengthLevel, analyze_entropy, classify_strength
from src.core.mutation_simulator import MutationResult, simulate_mutation_attack
...
def build_warnings(
    pattern: PatternResult, breach: BreachResult, mutation: MutationResult
) -> list[str]:
    warnings = []
    if breach.checked and breach.is_breached:
        warnings.append(
            f"Found in {breach.breach_count:,} known data breaches — do not use this password"
        )
    ...
    if pattern.has_personal_info:
        warnings.append(
            f"Contains part of your username/email: {', '.join(pattern.personal_info_matches)}"
        )
    if pattern.is_common_password:
        warnings.append(
            f"One of the most common passwords (matched: {pattern.matched_common_word})"
        )
    elif mutation.vulnerable:
        warnings.append(
            f"A simple variation of a common password (base word: {mutation.base_word})"
        )
    ...
    if pattern.has_keyboard_walk:
        warnings.append(
            f"Contains a predictable keyboard pattern: {', '.join(pattern.keyboard_matches)}"
        )
...
def build_score_breakdown(
    ...
    items = [
        ScoreBreakdownItem(
            "Base entropy (length \u00d7 character-set size)", round(raw_entropy_bits, 2)
        )
    ]
    ...
    if pattern.is_common_password:
        items.append(
            ScoreBreakdownItem(
                f"Common password match ({pattern.matched_common_word})", -COMMON_PASSWORD_PENALTY
            )
        )
    elif mutation_penalty > 0:
        items.append(
            ScoreBreakdownItem(
                f"Variation of a common word ({mutation.base_word})", -mutation_penalty
            )
        )
    if pattern.has_sequential:
        n = len(pattern.sequential_matches)
        items.append(
            ScoreBreakdownItem(f"Sequential pattern ({n}x)", -SEQUENTIAL_PENALTY_PER_MATCH * n)
        )
    if pattern.has_keyboard_walk:
        n = len(pattern.keyboard_matches)
        items.append(
            ScoreBreakdownItem(f"Keyboard pattern ({n}x)", -KEYBOARD_PENALTY_PER_MATCH * n)
        )
    if pattern.has_repeated_chars:
        n = len(pattern.repeated_matches)
        items.append(
            ScoreBreakdownItem(f"Repeated characters ({n}x)", -REPEATED_PENALTY_PER_MATCH * n)
        )
    if pattern.has_personal_info:
        n = len(pattern.personal_info_matches)
        items.append(
            ScoreBreakdownItem(f"Personal info match ({n}x)", -PERSONAL_INFO_PENALTY_PER_MATCH * n)
        )
...
def analyze_password(
    ...
    mutation_penalty = (
        MUTATION_PENALTY
        if mutation_result.vulnerable and not pattern_result.is_common_password
        else 0.0
    )
    effective_bits = max(
        0.0, entropy_result.entropy_bits - pattern_result.penalty_bits - mutation_penalty
    )
    ...
    breakdown = build_score_breakdown(
        entropy_result.entropy_bits, pattern_result, mutation_result, mutation_penalty
    )
